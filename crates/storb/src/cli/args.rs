...
pub fn common_args() -> Vec<Arg> {
    vec![
        ...
        Arg::new("dht.port")
            ...
            .action(ArgAction::Set)
            .global(true),
        Arg::new("no_bootstrap")
            .long("no-bootstrap")
            .help("Start node without bootstrap. Starts a new swarm")
...
pub fn get_neuron_config(args: &ArgMatches, settings: &Settings) -> Result<BaseNeuronConfig> {
    ...
    let no_bootstrap = *get_config_value!(args, "no_bootstrap", bool, settings.dht.no_bootstrap);
